...
static int cmpX(const void* a, const void* b)
{
	...
	return (int)((*(BallPosition*)a).distance - (*(BallPosition*)b).distance);
}

static int cmpY(const void* a, const void* b)
{
	//return (*(BallPosition*)a).fieldCoords.y - (*(BallPosition*)b).fieldCoords.y;
	return (int)((*(BallPosition*)a).heading - (*(BallPosition*)b).heading);
...
std::pair<unsigned, BallPosition*>  kdNode2D::nearest(const BallPosition &point, int depth)
{
	...
	*/
	const int tmp = (int)(depth == 0 ? p->distance - point.distance : p->heading - point.heading);
	...
	if (right.second == NULL && left.second == NULL){ // both sides are used, return itself
		cv::Point2d r = cv::Point2d(p->distance, p->heading);
		r.x -= point.distance;
		r.y -= point.heading;
		if (p->isUpdated) {
			//std::cout << "point " << p->id << " is used" << std::endl;
